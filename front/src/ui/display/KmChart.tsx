/**
 * DX3 (mejora futura ejecutada): gráfica de km ÚNICA para ambas apps — antes
 * el gestor veía esta versión rica (overlay anual/contrato, tooltip, hoy) y el
 * conductor una mínima de 39 líneas del mismo dato.
 */
import { useState } from 'react'

import { useAppLang, type AppLanguage } from '../../i18n/index.ts'
import { useUiCopy } from '../copy.ts'

/** Forma mínima de una lectura (las apps pasan su KmReading tal cual). */
...
export function KmChart({ readings, overlay }: { readings: ChartReading[]; overlay?: KmChartOverlay }) {
  // Índice del punto con el ratón encima (para el bocadillo). Hooks siempre arriba.
  const [hover, setHover] = useState<number | null>(null)
  const copy = useUiCopy().kmChart
  const lang = useAppLang()
  const locale = LOCALE[lang]
  const km = (value: number) => `${value.toLocaleString(locale)} km`
