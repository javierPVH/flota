...
| POST   | `/api/v1/incidents/{id}/manage/` | gestión | **Fase 2 del ciclo** (gestión): `{workshop_postal_code}` (5 cifras, obligatorio) — la ubicación preferente desde la que buscar el taller más cercano; deja la incidencia EN CURSO. Documentos y fotos van por `/documents/` ligados a la incidencia |
| POST   | `/api/v1/incidents/{id}/resolve/` | gestión | **Fase 3 del ciclo** (solución): `{resolution_date, observations?, overcost?}` — la fecha es **obligatoria** (ni futura ni anterior a la avería); el servidor calcula `downtime_days` desde la fecha de la avería y guarda todo en `details.resolution`. CIERRA la incidencia (R3-41) |
| CRUD   | `/api/v1/fuel-consumptions/` | ✔ᵃ | **Consumo mensual de combustible** (GAP-2): litros (e importe) por vehículo y mes, día 1 normalizado, un mes por vehículo entre filas vivas (la duplicada → 400 de campo). El CRUD (crear/editar/borrar, con `period` y `source` libres) es de **gestión**; el conductor registra solo por `add/` (R3-38, append-only como las lecturas de km). Alimenta el informe `fuel` |
