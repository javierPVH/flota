...
| GET/POST | `/api/v1/document-deletion-requests/` | gestión o conductor | **Peticiones de borrado de un documento**: la papelera de la app de campo NO borra, abre una petición (`{document, reason}`) y el documento se queda marcado (`Document.deletion_pending`). Idempotente por documento (una viva por documento) y acotada por `readable_documents`: no se puede pedir el borrado de lo que no se puede leer. Cada uno ve las suyas; la gestión, todas |
| POST   | `/api/v1/document-deletion-requests/{id}/resolve/` | admin | La decide: `{decision: "delete"\|"hide"\|"reject", note?}`. **delete** desactiva el documento (N7 → erratas, con el motivo como razón de la baja), **hide** lo deja en la flota pero `protected` y con `responsible` = quien decide (deja de verlo el campo, lo sigue viendo gestión) y **reject** no toca el documento. Ya resuelta → 400 |
